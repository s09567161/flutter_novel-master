  ...
  static List<ReaderChapterPageContentConfig> getChapterPageContentConfigList(
    ...
    while (paragraphs.length > 0) {
      ...
      while (currentHeight < height) {
        ...
        /// 当前展示内容 若存在图片url，会附带url字符串
        String currentParagraphContent = tempContent;

        /// 检查是否带有url
        String regexString = r'img=(.+?)=img';
        RegExp regExp = new RegExp(regexString);
        Iterable<Match> matches = regExp.allMatches(tempContent);

        /// 图片段落
        if (matches.length > 0) {
          currentParagraphContent = '';
          ///带有url，获取url中带有的宽高
          List<Match> imgUrlList = matches.toList();
          for (var i = 0; i < imgUrlList.length; i++) {
            // 计算图片
            String url = imgUrlList[i].group(0);

            //获取图片高度
            url = url.replaceAll('img=', '');
            url = url.replaceAll('=img', '');
            Map urlMap = json.decode(url);
            ...
            if (imgH + currentHeight > height) {
              //当前页不够显示,url放至下一页
              paragraphs[0] = imgUrlList[i].group(0);
              currentHeight = height;
              currentParagraphContent = tempContent.substring(0,tempContent.length - imgUrlList[i].input.length);
              //结束循环
              break;
            } else {
              currentHeight += imgH;
              currentHeight += paragraphSpacing;
              currentParagraphContent = currentParagraphContent + imgUrlList[i].group(0);
          ...
          /// 纯文本段落如果当前段落的内容展示不下，那么裁剪出展示内容，剩下内容填回去,否则移除顶部,计算下一个去
