...
class HtmlParser {
  ...
  HtmlParser(input,
    ...
    //  html5lib did not implement the no script parsing mode
        ...
        if (type == TokenKind.parseError) {
          ...
          switch (type) {
            ...
            case TokenKind.startTag:
              StartTagToken nToken = newToken as StartTagToken;
              if(nToken.name == 'img') {
                newToken = localPhase.processImageTag(newToken);
              }else {
  ...
  void adjustForeignAttributes(StartTagToken token) {
    //  I don't like mixing non-string objects with strings in
...
class Phase {
  ...
  Token processImageTag(StartTagToken token) {
    String josnNameString = json.encode(token.data);
    tree.insertText('img=$josnNameString=img', token.span);
    return null;
  }
...
class InBodyPhase extends Phase {
  ...
  void endTagFormatting(EndTagToken token) {
    ...
    //  the comments here don't match the numbered steps in the
...
class InForeignContentPhase extends Phase {
  //  this is sorted so we could binary search.
