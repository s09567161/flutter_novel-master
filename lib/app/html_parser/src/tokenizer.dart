...
//  we could use a better data structure here like a trie, if
...
class HtmlTokenizer implements Iterator<Token> {
  //  a lot of these could be made private
  ...
  //  the type should be "Predicate" but a dart2js checked mode
  ...
  //  ideally this would be a LinkedHashMap and we wouldn't add
  ...
  bool _tokenIsAppropriate() {
    //  this should use case insensitive compare instead.
  ...
  bool afterDoctypeNameState() {
    ...
    if (isWhitespace(data)) {
      ...
      if (data == "p" || data == "P") {
        //  would be nice to have a helper for this.
        var matched = true;
  ...
  bool cdataSectionState() {
    ...
    while (true) {
      ...
      data.add(ch);
      //  it'd be nice if we had an easier way to match the end,
