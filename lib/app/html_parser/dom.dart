...
abstract class Node {
  ...
  //  consider using an Expando for this, and put it in
  ...
  //  should this be a property or remove?
...
class DocumentType extends Node {
  ...
  String toString() {
    if (publicId != null || systemId != null) {
      //  the html5 serialization spec does not add these. But
...
//  Elements should have a pointer back to their document
class Element extends Node with _ParentNode, _ElementAndDocument {
  ...
  //  this is from dart:html _ElementFactoryProvider...
  //  have a look at fixing some things in dart:html, in
  // particular: is the parent tag map complete? Is it faster without regexp?
  //  for our version we can do something smarter in the parser.
  // All we really need is to set the correct parse state.
  factory Element.html(String html) {
    // this method can be made more robust and performant.
  ...
  set innerHtml(String value) {
    nodes.clear();
    //  should be able to get the same effect by adding the
    // fragment directly.
  ...
  static String _getSerializationPrefix(String uri) {
    ...
    //  the spec doesn't define "qualified name".
...
//  fix this to extend one of the corelib classes if possible.
// (The requirement to remove the node from the old node list makes it tricky.)
//  is there any way to share code with the _NodeListImpl?
class NodeList extends ListProxy<Node> {
  ...
  //  These aren't implemented in DOM _NodeListImpl, see
...
//  this was copied from dart:html
//  "implements List<Element>" is a workaround for analyzer bug.
class FilteredElementList extends IterableBase<Element>
  ...
  //
  // we don't always need to create a new list. For example
  ...
  //  this should be typed Element, but we currently run into a
